		 ...
		 std::list<Powerup*>& powerups, std::list<Wooden_box*>& wooden_boxes, std::list<Solid_box*>& solid_boxes)
{
    ...
    bool at_mid{(((int)old_position.x) % 50 < speed && ((int)old_position.y) % 50 < speed) ||
                (((int)old_position.x) % 50 > 50 - speed && ((int)old_position.y) % 50 > 50 - speed)};
	...
	score_assigner(wooden_boxes, up_score, down_score, right_score, left_score, up, right);
	score_assigner(solid_boxes, up_score, down_score, right_score, left_score, up, right);
